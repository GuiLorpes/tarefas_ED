...
class Lista:
    ...
    def __diminui(self):
        capacidade = int(len(self.valores) // 2)
        valores = array(capacidade, 0)
        for i in range(self.num_itens()):
            valores[i] = self.valores[i]
        self.valores = valores
    ...
    def acrescenta(self, item: int):
        ''' Adiciona um *item* no final da lista '''
        self.insere(self.tamanho, item)

    def listas_iguais(self, lst: Lista) -> bool:
        '''
        Compara a lista *self* e *lst* e verifica se ambas são iguais
        Exemplos:
        >>> a = Lista()
        >>> a.acrescenta(6)
        >>> a.acrescenta(7)
        ...
        '''
        r = False
        if self.num_itens() == lst.num_itens():
            for i in range (self.num_itens()):
